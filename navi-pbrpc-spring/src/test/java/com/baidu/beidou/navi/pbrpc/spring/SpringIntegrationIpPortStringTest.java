 ...
 * @author zhangxu
 */
@ContextConfiguration(locations = {"classpath*:ipportstring/applicationContext.xml"})
public class SpringIntegrationIpPortStringTest extends AbstractJUnit4SpringContextTests {
    ...
    @Test
    public void testDoSmthTimeout() {
        try {
            Demo.DemoRequest.Builder req = Demo.DemoRequest.newBuilder();
            req.setUserId(1);
            Demo.DemoResponse response = demoService.doSmthTimeout(req.build());
            System.out.println(response);
        } catch (Exception e) {
            assertThat(e.getClass().getSimpleName(), is("HAPbrpcException"));
            assertThat(e.getCause().getMessage(), is("java.io.IOException: Read timed out"));
            return;
        }
        fail();
    }
